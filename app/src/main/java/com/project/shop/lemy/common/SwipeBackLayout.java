...
public class SwipeBackLayout extends ViewGroup {
    ...
    private class ViewDragHelperCallBack extends ViewDragHelper.Callback {
        ...
        public int clampViewPositionVertical(View child, int top, int dy) {
            ...
            if (dragEdge == DragEdge.TOP && !canChildScrollUp() && top > 0) {
                ...
                result = Math.min(Math.max(top, topBound), bottomBound);
            } else if (dragEdge == DragEdge.TOP_BOTTOM) {
        ...
        public int clampViewPositionHorizontal(View child, int left, int dx) {
            ...
            if (dragEdge == DragEdge.LEFT && (locked || !canChildScrollRight()) && left > 0) {
        ...
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            Log.e("Tuan1", "" + yvel + " " + verticalDragRange);
            ...
            Log.e("Tuan1", "isBack" + isBack);

            int finalLeft;
            int finalTop;
            switch (dragEdge) {
                case LEFT:
                    finalLeft = isBack && xvel>=0 ? horizontalDragRange : 0;
    ...
    private boolean backBySpeed(float xvel, float yvel) {
        switch (dragEdge) {
            ...
            case TOP_BOTTOM:
                if (Math.abs(yvel) > Math.abs(xvel) && Math.abs(yvel) > AUTO_FINISHED_SPEED_LIMIT) {
                    return (!canChildScrollUp() && yvel<0) || (!canChildScrollDown() && yvel>0);
                }
                break;
            case LEFT:
            case RIGHT:
